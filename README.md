...
- 知识库创建、修改、删除与文档管理
- PDF、DOCX、Markdown、TXT、CSV、HTML 批量上传
...
当前多文档业务集包含 20 道订单、支付、物流、退换货和产品资料问题，并覆盖同义问法、跨文档问题、干扰问题与不可回答问题。最近一次单轮诊断结果为检索命中率 `90%`、引用准确率 `88.89%`、回答忠实度 `75.10%`、平均延迟 `12.56 秒`；忠实度尚未达到正式三轮门禁要求，需继续优化后再作为阶段验收结果。
...
- PDF 仅支持可提取文本，尚未实现扫描件 OCR、复杂表格恢复和图片理解。
